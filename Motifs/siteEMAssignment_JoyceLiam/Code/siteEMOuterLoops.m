...
MM[fastaFile_, motifLength_, pseudocountWeight_, maxIterations_, accuracy_, 
		includeReverseStrand_:False, numMotifsToFind_:1, outputFile_:"MMOutput"]:=
		
	Module[{input, inputLengths, erasers, baseCounts, backgroundFreqs, motifPseudocounts, 
		randomInitialPFM, initialMotifPrior, results={} },
		(*Initialization*)
		...
		(*In the published paper they iterate through many combinations of initializations
		  for the PFM and the prior, or expected number of motif occurrences. They say this
		...
		Do[AppendTo[results, MMIteration[input, motifLength, motifPseudocounts, initialMotifPrior, 
				randomInitialPFM, backgroundFreqs, maxIterations, accuracy, erasers]];
...
(* MMIteration returns one motif. It makes use of the erasers to avoid finding previously reported motifs. 
   MMIteration calls updateProbs repeatedly until either convergence (the PFMs are haven't changed much
...
MMIteration[input_, motifLength_, motifPseudocounts_, initialMotifPrior_, randomInitialPFM_,backgroundFreqs_, 
	...
	Module[{PFM=randomInitialPFM, oldPFM, motifPrior=initialMotifPrior, posteriors},
		Do[oldPFM=PFM;
